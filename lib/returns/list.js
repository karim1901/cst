 ...
 * @param {{shippingStatus?: string, validation?: string, page?: number, pageSize?: number}} options
...
export async function queryReturnsForMerchant(merchantId, options = {}) {
  const { shippingStatus = "all", validation = "all", page = 1, pageSize = 20 } = options;
...
export async function queryOrdersForSection(merchantId, options = {}) {
  ...
  if (section === ORDER_LIFECYCLE_SECTIONS.RETURNS) {
    return queryReturnsForMerchant(merchantId, { shippingStatus, validation, page, pageSize });
  }
 ...
 * Delivered (X) / Returns (X)". Deliberately independent of whichever
 * sub-filters (employee, delivery date, shipping status, validation) are
 * currently applied within a section: these are the section totals, the
 * same way a tab badge normally works. 3 small, indexed count queries run
 * together, never a full document fetch.
 */
export async function getSectionCounts(merchantId) {
  await connectToDatabase();
  ...
  const [all, delivered, returns] = await Promise.all([
    ...
    Order.countDocuments({ merchantId, deliveredAt: { $ne: null } }),
    withCollation(

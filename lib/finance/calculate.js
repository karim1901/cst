 ...
 * (`validatedReturnValueCents`): together they partition the total monetary
 * VALUE (`order.price`, the same canonical field revenue uses) of the
 ...
 * to any cost (Profit unchanged). An unknown Quick price (priceSource
 * "unknown") is NOT fabricated as 0 in either figure: excluded from the
 * total, counted in `returnValueUnknownPriceOrders` /
 * `validatedReturnValueUnknownPriceOrders`. See lib/finance/return-value.js.
 *
...
function emptyDayRow(date) {
  return {
    ...
    // Return Value / Validated Return Value: total monetary VALUE (order
    // price) of return-category orders, split by validation state — both
    // ADDITIVE reporting metrics, never fed into profit/cost.
    returnValueCents: 0,
...
export async function calculateMonthlyFinancials(merchantId, provider, period) {
  ...
  for (const order of providerOrders) {
    ...
    // fabricates an unknown Quick price.
    ...
    row.validatedReturnValueUnknownPriceOrders += vrv.unknownPriceOrders;
  ...
  const totals = {
    ...
    // totalCost/profit. They partition the Return bucket's known-price
    // value: returnValueCents + validatedReturnValueCents === SUM(price)
    // over every return-bucket order whose price is usable.

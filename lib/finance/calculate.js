 ...
 *
 * Employee Commission (`employeeCommissionCents`): a real operating expense
 * — what this merchant owes their team for the month, NOT a second
 * commission calculation. This function calls the exact SAME authoritative
 * service the Commission page itself uses,
 * `computeCommissionReport({merchantId, period, provider})`
 * (lib/commission/report.js), and sums every employee's `totalCommission`
 * (DH) into cents — no order-level unit math, no threshold/rate logic, no
 * tracking-number-month re-derivation lives here; all of that stays
 * exclusively in lib/commission/{report,calculate}.js. Guaranteed to
 * reconcile with the Commission page for the identical
 * merchant/provider/period because it IS that page's own calculation, run
 * once more with the same three inputs (never a second/parallel algorithm
 * that could drift).
 *
 * MONTH-LEVEL ONLY, deliberately, the SAME precedent `otherExpenseCents`
 * already established (see its own comment above): an employee's
 * commission RATE is decided by comparing their WHOLE MONTH'S total units
 * against their threshold (lib/commission/calculate.js#computeEmployeeCommission)
 * — a single flat rate applied to every one of that employee's units for
 * the month, never a per-order or per-day rate. There is no existing,
 * reliable way to attribute a fraction of that monthly-decided rate back to
 * one specific calendar day without inventing a NEW allocation rule this
 * task explicitly warns against fabricating — so, like `otherExpenseCents`,
 * `employeeCommissionCents` is added to the MONTHLY `totals` only (Total
 * Costs, Net Profit, Net Profit — Delivered Only) and is NOT distributed
 * into the daily `days[]` rows (each day's own `employeeCommissionCents`
 * stays a structural `0`, matching how `otherExpenseCents` already behaves
 * per day — see `emptyDayRow` and the per-day totalCost loop below).
 *
 * Included in BOTH profit formulas identically (`profitCents` AND
 * `deliveredOnlyProfitCents`) — unlike product/shipping cost, there is no
 * "delivered vs. all orders" distinction to make here: commission is
 * ALREADY, by construction, generated exclusively from DELIVERED orders
 * (`commissionUnitsForOrder` returns 0 for anything not delivered) — the
 * one `employeeCommissionCents` figure already IS the delivered-only cost,
 * so both formulas add the exact same value rather than needing two
 * different commission figures.
 */
...
function emptyDayRow(date) {
  return {
    ...
    otherExpenseCents: 0,
    // Employee Commission is a MONTH-LEVEL-ONLY figure, the same established
    // precedent as `otherExpenseCents` immediately above (see module
    // comment) — this stays a structural 0 for every individual day; only
    // `totals.employeeCommissionCents` (assigned directly below, never
    // summed from these day rows) carries the real value.
    employeeCommissionCents: 0,
    orderRows: [],
  };
}
...
export async function calculateMonthlyFinancials(merchantId, provider, period) {
  ...
  // specific), this provider's city pricing, and this provider+period's
  // employee commission — the SAME authoritative Commission-page service
  // (lib/commission/report.js), run once here, never reimplemented (see
  // module comment's "Employee Commission" paragraph).
  const [productCostDocs, cityPricingDocs, adExpenseDocs, otherExpenseDocs, commissionReport] = await Promise.all([
  ...
  // Sum every employee's `totalCommission` (DH, the Commission page's own
  // unit) into integer cents — `null` for an employee with no commission
  // configuration (defensive fallback, see lib/commission/report.js) is
  // never treated as 0-owed; it is simply excluded from the sum, same as
  // this whole app's "unresolved is not a real zero" convention elsewhere.
  const employeeCommissionCents = sumCents(
    commissionReport.filter((e) => e.configured).map((e) => dhToCents(e.totalCommission) ?? 0)
  );
  ...
  const totals = {
    ...
    // Employee Commission — MONTH-LEVEL ONLY, assigned directly (never
    // summed from `days`, which all stay structurally 0 — see module
    // comment and `emptyDayRow`'s own comment for the full rationale).
  ...
  totals.totalCostCents =
    ...
    totals.employeeCommissionCents;
  ...
  // `profitCents` — never replaces it. `employeeCommissionCents` is the
  // SAME figure as above, not a second delivered-only commission
  // calculation — see module comment's "Included in BOTH profit formulas
  // identically" paragraph for why that is correct, not a shortcut.
  ...
  // Other expenses AND Employee Commission stay month-level only (never
  // distributed per day — see this function's own "Other expenses are a
  // MONTH-level deduction" and "Employee Commission" module comments), for
  // BOTH profit figures, so neither double-counts them.

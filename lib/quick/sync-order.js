...
export async function syncOneQuickOrder({
  ...
  if (existing) {
    ...
    // REVIVE: this exact tracking number was just confirmed to exist at
    // Quick (this function is only ever called after
    // lib/quick/lookup.js#lookupQuickParcel classified it "exists" — see
    // this module's own header comment) — if it was previously marked
    // provider-deleted (lib/orders/reconcile-stale-orders.js /
    // lib/quick/reconcile-current-month.js), the provider's current answer
    // is authoritative, so it becomes active again. Deletion itself is
    // NEVER decided here — only by those two modules' own explicit
    // "not_found" signal.
    if (
      ...
      await Order.updateOne(
        ...
        console.error("[syncOneQuickOrder] failed to revive", trackingNumber, "-", err?.message);
      });
    }

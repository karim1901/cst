 ...
 *                  lib/quick/sync-order.js#syncOneQuickOrder, which also
 *                  REVIVES it if it was previously marked provider-deleted
 *                  by this function), never touching an existing order's
 ...
 *                  not-found phrase) -> mark the local Order
 *                  `providerRecordStatus: "deleted"` (soft-delete — see
 *                  lib/orders/provider-record-status.js /
 *                  models/Order.js's own field comment: a physical delete
 *                  would erase real historical financial data), if one
 *                  exists. This is what makes a manually-deleted-at-Quick
 *                  order stop appearing as ACTIVE here too, while its
 *                  history is preserved and current Finance/Returns/
 *                  Dashboard/Commission queries (which all filter on
 *                  `ACTIVE_PROVIDER_ORDER_FILTER`) stop counting it.
 *   "unknown"   -> genuinely ambiguous even after lib/quick/lookup.js's own
 *                  retries (or the lookup call itself threw — a network/API
 *                  failure) -> NEVER mark deleted. Keep and display the
 *                  local order's last known state, if any. A temporary
 *                  hiccup must never be confused with a confirmed absence.
 */
      ...
      batchCounters.map(async (c) => {
        ...
        try {
          ...
          if (classification === "exists") {
            ...
            // Quick POSITIVELY confirms absence. `reconciliationAction`
            // decides whether there is anything to do — "markDeleted" for a
            // currently-active local order, "none" when it is already
            // marked deleted (nothing changed, and it must NOT be streamed
            // into the live list either way — a confirmed absence is never
            // shown, unlike the genuinely-ambiguous "unknown" case below).
            if (
              localMatch &&
              reconciliationAction({ classification, currentStatus: localMatch.providerRecordStatus }).action ===
                "markDeleted"
            ) {
              // Soft-delete the stale local mirror (never a physical delete
              // — see this module's own comment), scoped to this exact
              // owner+provider+tracking number (never a broader write) so
              // it stops counting as active anywhere.
              // `lastKnownStatus`/`deliveredAt` are left untouched.

...
export async function syncHistoricalOzonOrders(employeeId, options = {}) {
  ...
  for (const period of targetPeriods) {
    ...
    if (startCounter == null || startCounter < MONTH_BOUNDARY_COUNTER) {
      summary.periods.push({ period, foundAtOzon: 0, created: 0, updated: 0, skipped: 0, staleDeleted: 0 });
      continue;
    }
    ...
    // period's sync just used; only LOCAL orders already in Mongo for this
    // period are checked (never the full blind range), so this is cheap
    // even though `orders` above may be large.
    ...
    summary.periods.push({ period, foundAtOzon: orders.length, created, updated, skipped, staleDeleted });
  }
...
export async function syncHistoricalQuickOrders(employeeId, options = {}) {
  ...
  for (const period of targetPeriods) {
    ...
    if (startCounter == null || startCounter < MONTH_BOUNDARY_COUNTER) {
      summary.periods.push({ period, foundAtQuick: 0, created: 0, updated: 0, skipped: 0, staleDeleted: 0 });
      continue;
    }
    ...
    for (const item of found) {
      ...
      try {
        const { result } = await syncOneQuickOrder({
          merchantId,
          employeeId: owner.role === USER_ROLES.EMPLOYEE ? employeeId : null,
          createdByRole: owner.role,
          trackingNumber,
    ...
    summary.periods.push({ period, foundAtQuick: found.length, created, updated, skipped, staleDeleted });
  }

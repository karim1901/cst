...
export async function computeCommissionReport({ merchantId, period, employeeId }) {
  ...
  return employees.map((employee) => {
    ...
    // Provider transparency (item 10) — a per-provider ORDER COUNT
    // breakdown, purely additive: it changes nothing about which orders
    // count, the threshold, the rate, or the total commission below (all
    // still computed over `employeeOrders` exactly as before — see
    // lib/commission/calculate.js, untouched). This never double-counts:
    // every order in `employeeOrders` belongs to exactly one provider
    // (its own `provider` field), so the per-provider counts always sum to
    // `employeeOrders.length`.
    const ordersByProvider = employeeOrders.reduce((acc, order) => {

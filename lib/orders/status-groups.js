...
// Ozon's own accented French return statuses, plus their plain-English/
// unaccented equivalents — which is what makes this list already cover
// Quick Livraison's own documented values ("annule"/"refuse"/"retourne",
// unaccented) too: `normalizeStatus` strips accents before comparing, so
// "Annulé" and "annule" both normalize to the same token. Never invents new
// statuses beyond this — an unrecognized status
...
/**
 * THE one source of truth for these 3 buckets' display labels — this
 * app's own established status vocabulary ("Tous"/"Livré"/"Progress"/
 * "Retour"), deliberately French/English, NEVER translated into Arabic
 * (or re-translated anywhere else) — every place in the UI that shows
 * these words (the Orders page's StatusFilter, the Returns page's section
 * tabs, Dashboard/Finance stat cards) imports THIS constant rather than
 * hardcoding the words locally or routing them through i18n, so there is
 * exactly one place this vocabulary is defined and it can never drift out
 * of sync or get accidentally translated by a future i18n change.
 */
export const STATUS_FILTER_LABELS = Object.freeze({
  [ORDER_STATUS_FILTERS.ALL]: "Tous",
  [ORDER_STATUS_FILTERS.DELIVERED]: "Livré",
  [ORDER_STATUS_FILTERS.PROGRESS]: "Progress",
  [ORDER_STATUS_FILTERS.RETURN]: "Retour",
});
...
/**
 * Generic (any-provider) status badge tone — 3 broad buckets (green =
 * delivered, red = cancelled/refused/returned, amber = still in progress,
 * zinc = no status yet) built entirely from this module's own centralized
 * classification above. Used by Quick Livraison's order list
 * (app/_components/orders/QuickOrdersList.jsx, via StatusBadge) now that
 * item 6's documented status values ("DELIVERED", "annule"/"refuse"/
 * "retourne", ...) confirm Quick maps cleanly into these same categories.
 * Ozon Express keeps its own richer, provider-specific tone function
 * (lib/ozon/status.js#statusBadgeTone, UNCHANGED) since its exact status
 * vocabulary supports finer distinctions (dispatched/new/awaiting-pickup/
 * ...) this generic 3-bucket classification would lose.
 */
export function genericStatusBadgeTone(provider, status) {
  if (!status) return "zinc";
  if (isDeliveredDisplayStatus(provider, status)) return "green";
  if (isReturnStatus(status)) return "red";
  return "amber";
}
